                            ...
                            <table class="table">
                                ...
                                @foreach($book as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->title }}</td><td class="d-none">{{ $item->content }}</td><td>{{ $item->year }}</td><td>{{ $item->price }}</td><td>{{ $item->isbn }}</td><td>{{ $item->author_id }}</td><td>{{ $item->publisher_id }}</td><td > <img src="{{ asset("/storage/{$item->photo}")   }}" width="140" height="180"></td>
                                        <td>
                                            <a href="{{ url('/book/' . $item->id) }}" title="View Book"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> View</button></a><br>
                                            <a href="{{ url('/book/' . $item->id . '/edit') }}" title="Edit Book"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a><br>
